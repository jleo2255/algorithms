...
static int qksort_random_unsorted_arr[1000000];
...
void fill_qksort_random_unsorted_arr(void)
{
	...
	for(int i = 0; i < 1000000; i++) qksort_random_unsorted_arr[i] = rand();
}
...
Test(quick_sort, big_ascending_sort, .init = fill_qksort_random_unsorted_arr )
{
	...
	qksort(&qksort_random_unsorted_arr, 1000000, sizeof(int), 0, (1000000 - 1), &qksort_compare_ints);

	qksort_end_times[0] = clock();
	
	double elapsed_time = (qksort_end_times[0] - qksort_start_times[0])/(double)CLOCKS_PER_SEC;

	printf(KYEL "Quicksort: Sorting 1000000 ints took approximately %f seconds.\n", elapsed_time);
}
